...
def Rand_Transforms(imgs,
    ...
    #print(imgs.max())
    pil_imgs = [Image.fromarray(x*255) for x in imgs]
    
    w, h = pil_imgs[0].size
    #print(w, h)
    #print(pil_imgs[100])
    #pil_imgs[100].convert("L").save('test.jpg')
    #cv2.imwrite('test.jpg', imgs[100]*255)
    # Affine Transforms
    ...
    pil_imgs = [colorop(x.convert("L"), bright, contrast) for x in pil_imgs]
    
    imgs = np.asarray([np.asarray(x,dtype=np.float32) for x in pil_imgs], dtype=np.float32)
