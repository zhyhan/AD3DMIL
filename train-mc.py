...
Validset = CTDataset(datalist=VAL_DATA_ROOT, target='valid')
...
for e in range(RESUME_EPOCH, TRAIN_EPOCH):
    ...
    for i, (all_F, all_L, all_info) in enumerate(tqdm(TrainLoader)):
        ...
        labels = all_L.cuda()
        preds, _ = model([all_F.cuda()])

        train_loss = criterion(preds, labels)
        train_loss.backward()
        optimizer.step()
        
        train_acc = topk_accuracies(preds, labels, [1])[0]
    ...
    t_loss.append(Ece), t_acc.append(Epoch_Acc)
    ...
    if e % SNAPSHOT_FREQ == 0 or e >= TRAIN_EPOCH-1:
        ...
        v_loss.append(Ece), v_acc.append(EAcc)
    ...
    to_save = {'train_acc':t_acc, 'train_loss':t_loss, 'val_acc':v_acc, 'val_loss':v_loss}
    np.save('train_loss_acc.npy', to_save) 